# -*- coding: utf-8 -*-
"""Kernel2A_Kernel2B.ipynb

Automatically generated by Colaboratory.

Original file is located at
    https://colab.research.google.com/drive/1zLXJd5BhkHGsm6MNOfCu0Czx2PRADYDP
...
def get_particles_chart(X, X_svgd=None):
    ...
    chart = alt.Chart(df).mark_circle().encode(x='x:Q',y='y:Q', color=alt.Color('id:N', legend=None) )
...
- [ ] grad must use automatic gradient
...
class Kernel2b:
  #kernel with fixed inputs
    ...
    def forward(self, input_1, input_2):
        ...
        # kappa_grad = torch.matmul(input_2.unsqueeze(-1),torch.ones((self.num_particles, self.particles_dim)).to(device).unsqueeze(1))*C
...
# X = torch.tensor([[1.0, 2.0], [-1.0, 1.0], [0.5, 2.3]]).to(device)
# a = Kernel2b(3,2)
# kappa, kappa_grad = a.forward(X,X)

# C1 = max(torch.linalg.norm(X, dim=1).pow(-1))
# C2 =  min(torch.linalg.norm(X, dim=1).pow(-1))
# print(C1, C2)

"""if we have this kernel

$k(\vec x, \vec y) = \frac {\vec x^T \vec y}{c c}$ where $\vec x =\begin{bmatrix} x_{1,1} & x_{1,2} \\ x_{2,1} & x_{2,2} \\ x_{3,1} & x_{3,2}\end{bmatrix} = \begin{bmatrix}\vec x_1 \\ \vec x_2 \\ \vec x_3 \end{bmatrix}$

so kappa should have the following format

$\begin{bmatrix}k(\vec x_1, \vec x_1) & k(\vec x_1, \vec x_2) &k(\vec x_1, \vec x_3) \\ k(\vec x_2, \vec x_1) & k(\vec x_2, \vec x_2) &k(\vec x_2, \vec x_3) \\ k(\vec x_3, \vec x_1) & k(\vec x_3, \vec x_2) &k(\vec x_3, \vec x_3)\end{bmatrix} = \begin{bmatrix} k_{1,1} & k_{1,2} & k_{1,3} \\ k_{2,1} & k_{2,2} & k_{2,3} \\ k_{3,1} & k_{3,2} & k_{3,3} \end{bmatrix}$

Generally $\nabla_{\vec x} k(\vec x, \vec y) = \frac {\vec y}{c^2}$

$\begin{bmatrix} \begin{bmatrix} \vec x_1 & \vec x_1\end{bmatrix} & \begin{bmatrix} \vec x_2 & \vec x_1\end{bmatrix} & \begin{bmatrix} \vec x_3 & \vec x_1\end{bmatrix} \\ \begin{bmatrix} \vec x_1 & \vec x_2\end{bmatrix} & \begin{bmatrix} \vec x_2 & \vec x_2\end{bmatrix} & \begin{bmatrix} \vec x_3 & \vec x_2\end{bmatrix} \\ \begin{bmatrix} \vec x_1 & \vec x_3 \end{bmatrix} & \begin{bmatrix} \vec x_2 & \vec x_3 \end{bmatrix} & \begin{bmatrix} \vec x_3 & \vec x_3 \end{bmatrix}\end{bmatrix}$
"""
...
# torch.matmul(X.unsqueeze(1), torch.ones((3,2, 2)).to(device))

"""class Kernel2c:
    def __init__(self, num_particles, dim, vector):
        self.num_particles = num_particles
        self.particles_dim = dim
        self.norm = torch.linalg.norm(vector, dim = 1)
        self.kappa_func = lambda x, y: torch.matmul(x*(self.norm.pow(-1)).unsqueeze(1), (y*(self.norm.pow(-1)).unsqueeze(1)).t())
        self.y = None
        self.Y = None
        self.first = None
        self.second = None
    ...
    def forward(self, input_1, input_2):
        kappa = (self.kappa_func(input_1, input_2) + 1).to(device)
        V = self.norm.pow(-1).unsqueeze(1).to(device)
        self.y = ((torch.ones(self.num_particles, self.particles_dim)).to(device)*V).to(device)
        self.Y = (input_1*(V)).to(device)
        self.first = torch.matmul(self.y, self.Y.t())
        self.second = torch.matmul(self.Y, self.y.t()) #[a, b, c] * [A, B, C]^T
        kappa_grad = torch.cat((self.first.unsqueeze(-1), self.second.unsqueeze(-1)), dim=2)
        # indices = np.arange(0,self.num_particles)
        # kappa_grad = torch.permute(torch.matmul(V.unsqueeze(0).unsqueeze(-1), torch.permute(input_1*V.unsqueeze(1), (-1,0)).unsqueeze(1)), (-1, -2, 0))
        # kappa_grad[indices, indices] = 0
        # kappa_grad = ((input_1*V.unsqueeze(1)).unsqueeze(1))*(V.view(-1, 1).expand(self.num_particles,self.num_particles).unsqueeze(2))
...
class Entropy_toy():
    ...
    def __init__(self, P, K, optimizer, num_particles, particles_dim, with_logprob, num_svgd_steps, tb_logger, calculate_other_lines = True):
        ...
        #intermediate gaussian parameters
        # self.mu_0 = X.mean()
    ...
    def compute_logprob(self, phi, X, svgd_itr):
        ...
        '''
        ########## Uncomment when we need to visualize line 3
...
class intermediate_gauss():
    """
    Calculate intermediate gaussian parameters
    Inputs:
        optimizer: Gradient Ascent optimizer
        particles_dim: particles number of dimensions
        tb_logger: for tensorboard visualization
    """
    def __init__(self, particles_dim, optimizer, tb_logger,  Q, b):
        self.optim = optimizer
        self.particles_dim = particles_dim
        self.identity_mat =  torch.eye(self.particles_dim).to(device)
        self.t_log = tb_logger
        self.Q = Q
        self.b = b


    def compute_parameters(self, mu, covariance, with_norms, X):
        #compute alpha_mu
        c = min(torch.linalg.norm(X, dim=1).pow(-1))**2 #or max, experiment with both :3

        b = 1 if not with_norms else c
        a = b*torch.matmul(mu, mu)
        Q_inv = torch.linalg.inv(self.Q)

        alpha_mu_1 = b*torch.matmul(self.identity_mat - torch.matmul(Q_inv, covariance), mu)
        alpha_mu_2 = (a+1)*(torch.matmul(Q_inv, (mu - self.b)))
        alpha_mu = alpha_mu_1 - alpha_mu_2
        alpha_x = torch.zeros(X.shape[0], X.shape[1]).to(device)

        beta_cov_1 = 2*covariance
        beta_cov_2 = torch.matmul(covariance ,torch.matmul(covariance + torch.matmul(mu.unsqueeze(-1), (mu - self.b).unsqueeze(0)), Q_inv))
        beta_cov_3 = torch.matmul(Q_inv, torch.matmul(covariance + torch.matmul((mu - self.b).unsqueeze(-1), mu.unsqueeze(0)), covariance))

        beta_cov = b*(beta_cov_1 - beta_cov_2 - beta_cov_3)

        #next_mu
        mu_next = self.optim.step(mu, alpha_mu)
        mu = mu_next

        #next covariance_matrix
        cov_next = self.optim.step(covariance, beta_cov)
        covariance = cov_next
        #the next tensor according to this
...
def my_experiment(dim, n_gmm, num_particles, num_svgd_step, kernel_sigma, gmm_std, lr, mu_init, sigma_init, tb_logger,Project_name,plot, calculate_other_lines = False):
    ...
    b = torch.Tensor([-0.6871,0.8010]).to(device)
    Q = 5 *torch.Tensor([[0.2260,0.1652],[0.1652,0.6779]]).to(device)

    mu_0 = (torch.mean(X_init, dim=0))
    covariance_0 = (torch.matmul(X_init.unsqueeze(-1), X_init.unsqueeze(1))).mean(0) - torch.matmul(mu_0.unsqueeze(-1), mu_0.unsqueeze(0))
    ...
    # experiment = Entropy_toy(gauss, Kernel1(num_particles), Optim(lr), num_particles=num_particles, particles_dim=dim, with_logprob=True, num_svgd_steps=num_svgd_step, tb_logger=tb_logger, calculate_other_lines = calculate_other_lines)
    ...
    parameters = intermediate_gauss(particles_dim=dim, optimizer=Optim(lr), tb_logger=tb_logger,  Q=Q, b=b)
    ...
    def main_loop(alg, X, steps, mu, covariance):
        """
        ...
        for t in range(steps):
            ...
            covariance_=(torch.matmul(X.unsqueeze(-1), X.unsqueeze(1))).mean(0) - torch.matmul(mean_.unsqueeze(-1), mean_.unsqueeze(0))
            print('__________________',t)
            mean_svgd, covariance_svgd = parameters.compute_parameters(mean_, covariance_, with_norms=True, X= X.detach() )
            X, phi_X = experiment.step(X, alg,  svgd_itr=t)
            # print('__________________mu_0=',X.mean())
            if plot:
            ...
            # from paper
            mu, covariance= parameters.compute_parameters(mu, covariance, with_norms=False, X= X.detach() )
            # normalized from paper
            mu_NORMS, covariance_NORMS= parameters.compute_parameters(mu, covariance, with_norms=True, X= X.detach() )


            tb_logger.add_scalars("Covariance_matrix", {
                                                         "no_NORMS_covariance.mean()":covariance.mean().item(),
                                                         "SVGD_covariance.mean()":covariance_svgd.mean().item(),
                                                         "ground_truth_Q.mean()": Q.mean().item(),
                                                         "Covariance_with_norms": covariance_NORMS.mean().item()},t)

            tb_logger.add_scalars("Covariance_matrix_WITH_NORMS_COMPONENTS", {"WITH_NORMS_covariance_0":covariance_NORMS[0][0].item(),
                                                        "WITH_NORMS_covariance_1":covariance_NORMS[0][1].item(),
                                                        "WITH_NORMS_covariance_2":covariance_NORMS[1][0].item(),
                                                        "WITH_NORMS_covariance_3":covariance_NORMS[1][1].item(),
                                                        "Q0": Q[0][0].item(),
                                                        "Q_1":Q[0][1].item(),
                                                        "Q_2":Q[1][0].item(),
                                                        "Q_3":Q[1][1].item()},
            ...
            tb_logger.add_scalars("Convergence_matrix_components_NO_NORMS", {
                                                                    "no_NORMS_covariance_0":covariance[0][0].item(),
                                                                    "Q0": Q[0][0].item(),
                                                                    "no_NORMS_covariance_1":covariance[0][1].item(),
                                                                    "Q_1":Q[0][1].item(),
                                                                    "no_NORMS_covariance_2":covariance[1][0].item(),
                                                                    "Q_2":Q[1][0].item(),
                                                                    "no_NORMS_covariance_3":covariance[1][1].item(),
                                                                    "Q_3":Q[1][1].item()}, t)

            tb_logger.add_scalars("Mu_components", {"NO_NORM_mu_x":mu[0].item(),
                                                    "b_x":b[0].item(),
                                                    "NO_NORM_mu_y":mu[1].item(),
                                                    "b_y":b[1].item(),
                                                    "WITH_NORM_mu_x":mu_NORMS[0].item(),
                                                    "WITH_NORM_mu_y":mu_NORMS[1].item()}, t)

            tb_logger.add_scalars("Mu", {"NO_NORMS_mu.mean().item()":mu.mean().item(),
                                         "SVGD_mean.mean().item()":mean_svgd.mean().item(),
                                         "WITH_norms.mean().item()":mu_NORMS.mean().item(),
                                         "b.mean().item()":b.mean().item()},t)

            tb_logger.add_histogram("Kernel_matrix_values", experiment.K_XX,t)
            tb_logger.add_scalar("negative_values_in_kernel", (experiment.K_XX < 0).sum().item() ,t)
    ...
    gt_entr, sampler_entr_svgd, charts_svgd, X, ent, conv = main_loop('svgd', X_init.clone(), steps=num_svgd_step, mu=mu_0, covariance= covariance_0)
...
sigma_k_param = {}
calculate_other_lines = False

# project name and logger
Project_name = "__Thursday_exp2.0___"+str(kernel_sigma)
...
print(Project_name)
print("*************************************")
# means, covariance_mx  = toy_exp(dim, num_particles, lr, num_steps, mu_init, sigma_init, tb_logger, Project_name)
